    ...
    public class SerilogConfig
    {
        public static ILogger CreateLogger()
        {
            ...
            var config = new LoggerConfiguration().
                MinimumLevel.Debug().
                WriteTo.Sink(new RollingFileSink(loggerFolder + "\\serilog-{Date}.json", new JsonFormatter(renderMessage: true), 1024 * 1024 * 100, 7), LogEventLevel.Debug).
                WriteTo.Sink(new RollingFileSink(loggerFolder + "\\serilog-{Date}-norender.json", new JsonFormatter(renderMessage: false), 1024 * 1024 * 100, 7), LogEventLevel.Debug).
                WriteTo.RollingFile(loggerFolder + "\\serilog-debug-{Date}.log", LogEventLevel.Debug, fileSizeLimitBytes: 1024 * 1024 * 100, retainedFileCountLimit: 7);
