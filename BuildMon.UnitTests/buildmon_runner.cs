...
namespace BuildMon.UnitTests
{
    ...
    public class buildmon_runner
    {
        ...
        public void given_single_build_item_when_constructed_should_display_that_item()
        {
            ...
            mockBuildSource.Setup(
                bs =>
                    bs.StartMonitoring(It.IsAny<Action<IEnumerable<IBuildItem>>>(),
                        It.IsAny<Action<IEnumerable<string>>>()))
                .Returns(Task.FromResult<IEnumerable<IBuildItem>>(items));

            var mockBuildDisplay = new Mock<IBuildDisplay>();

            var subject = new BuildMonRunner(Mock.Of<IMainWindow>(), new[] {mockBuildSource.Object},
                new[] {mockBuildDisplay.Object});
            subject.Start();
